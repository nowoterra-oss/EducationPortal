...
public class InternalExamService : IInternalExamService
{
    ...
    public async Task<ApiResponse<List<InternalExamDto>>> GetByStudentAsync(int studentId)
    {
        ...
        catch (Exception ex)
        {
            ...
            return ApiResponse<List<InternalExamDto>>.ErrorResponse($"�renci s1navlar1 getirilirken bir hata olu_tu: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<InternalExamDto>> UpdateAsync(InternalExamUpdateDto dto)
    {
        try
        {
            ...
            return ApiResponse<InternalExamDto>.SuccessResponse(examDto, "S1nav ba_ar1yla g�ncellendi");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error occurred while updating exam: {ExamId}", dto.Id);
            return ApiResponse<InternalExamDto>.ErrorResponse($"S1nav g�ncellenirken bir hata olu_tu: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<PagedResponse<ExamResultDto>>> GetResultsAsync(int examId, int pageNumber = 1, int pageSize = 10)
    {
        ...
        catch (Exception ex)
        {
            ...
            return ApiResponse<PagedResponse<ExamResultDto>>.ErrorResponse($"Sonu�lar getirilirken bir hata olu_tu: {ex.Message}");
        }
    }
    ...
    public async Task<ApiResponse<ExamResultDto>> AddResultAsync(ExamResultCreateDto dto)
    {
        try
        {
            ...
            if (student == null)
            {
                return ApiResponse<ExamResultDto>.ErrorResponse("�renci bulunamad1");
            }
            ...
            {
                return ApiResponse<ExamResultDto>.ErrorResponse("Bu �renci i�in s1nav sonucu zaten mevcut");
            }
